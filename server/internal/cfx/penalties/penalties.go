...
func GetPlayerPenalties(steamId string) ([]cfx_models.Penalties, error) {
	...
	err := db.CfxMariaDB.Client.Where("steamId = ?", steamId).Order("date DESC").Find(&penalties).Error
...
func IsPlayerBanned(steamId string) (*time.Time, error) {
	...
	err := db.CfxMariaDB.Client.Where("steamId = ? AND penalty = ?", steamId, cfx_models.BanPenalty).Order("date DESC").First(&penalty).Error
	if err != nil {
	...
	until := penalty.Date.Add(time.Duration(penalty.Length) * time.Hour * 24)
